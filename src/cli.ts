...
export class CLI {
    ...
    return AccountService.resolveProxyHostname(account.proxy).then(() => {
      ...
      const client = new Client(server, CLI.getBuildVersion(), account);
  ...
  public static getBuildVersion() {
    ...
    const minor = parametersFile.match(/MINOR_VERSION:String = "(\d+)/)[1];
    const newVersion = 'X' + build + '.' + minor;

    CLI.buildVersion = newVersion;

    console.log('Extracted build version', newVersion);
